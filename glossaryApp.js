...
{const s=document.createElement('style');s.textContent='#dictionaryContent section > div[data-multiline]{text-align:left}#dictionaryContent section > ul[data-multiline]{text-align:left}';document.head.append(s)}
...
function listField(label,values,extra='',linkable=false){if(!values?.length)return '';const multi=values.length>1?' data-multiline':'';return `<div class="term-field ${extra}"><div class="term-label">${esc(label)}</div><div class="term-value${multi}"><ul>${values.map(value=>`<li>${linkable?dictionaryText(value):esc(value)}</li>`).join('')}</ul></div></div>`}
...
function renderLearningCard(){
 ...
 const examText=term.exam?.length?term.exam.map(dictionaryText).join('<br>'):'';
