...
function renderLearningCard(){
 ...
 const reverseIsBulletList=reverseHints.length>=2&&reverseHints.some(value=>value.startsWith('・'));
 const reverseHintsHtml=reverseIsBulletList?`<span class="reverse-hints-list">${reverseHints.map(value=>value.startsWith('・')?`<span class="reverse-hint-item"><span class="reverse-hint-bullet">・</span><span class="reverse-hint-text">${esc(value.slice(1).trimStart())}</span></span>`:`<span class="reverse-hint-item reverse-hint-item-plain"><span class="reverse-hint-text">${esc(value)}</span></span>`).join('')}</span>`:reverseHints.map(esc).join('<br>');
