...
data.version='3.0.91';
quizData.version='3.0.91';
}
const APP_VERSION='3.0.91',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function recordCategoryRoundEvaluation(term,status){if(!term||!states[status])return;const terms=data.terms.filter(item=>item.category===term.category),round=categoryRoundData(term.category),answers=round.current.answers;answers[term.id]=status;if(Object.keys(answers).filter(id=>terms.some(item=>item.id===Number(id))&&states[answers[id]]).length===terms.length){const breakdown=categoryCurrentBreakdown(term.category,terms);round.completed++;round.history.push({round:round.completed,at:dateTime(),safe:breakdown.safe,caution:breakdown.caution,danger:breakdown.danger,unlearned:0,total:breakdown.total});round.current={answers:{},migrated:true}}saveLearning();flushLearning()}
