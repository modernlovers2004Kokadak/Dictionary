...
data.version='3.0.89';
quizData.version='3.0.89';
}
const APP_VERSION='3.0.89',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function categoryRoundData(category){const meta=categoryRoundMeta(),raw=meta[category];if(raw&&typeof raw==='object'&&!Array.isArray(raw)){raw.completed=Math.max(0,Number(raw.completed)||0);raw.history=Array.isArray(raw.history)?raw.history:[];return raw}return meta[category]={completed:0,history:[]}}
...
function saveCategoryRound(){if(sessionModeKey!=='categoryAll'||!session.length)return;const category=session[0].category,round=categoryRoundData(category),breakdown=categoryBreakdown(session);if(round.lastSessionId===sessionId)return;round.completed++;round.lastSessionId=sessionId;round.history.push({round:round.completed,at:dateTime(),safe:breakdown.safe,caution:breakdown.caution,danger:breakdown.danger,unlearned:breakdown.unlearned,total:breakdown.total});saveLearning();flushLearning()}
...
function categoryStats(){const categoryOrder=['感染症法','感染症予防','消毒法','公衆衛生','地域保健法','健康増進法','消費者基本法','皮膚','毛髪','香粧品成分','カッティング'],byCategory=new Map();data.terms.forEach(term=>{if(!byCategory.has(term.category))byCategory.set(term.category,[]);byCategory.get(term.category).push(term)});const rows=[...byCategory.entries()].map(([category,terms])=>({category,terms,...categoryBreakdown(terms),round:categoryRoundData(category)}));rows.sort((a,b)=>categoryOrder.indexOf(a.category)-categoryOrder.indexOf(b.category));return rows}
function startCategoryWeak(category){const categoryTerms=data.terms.filter(term=>term.category===category);if(!categoryTerms.length){showStatusMessage(`「${category}」に該当する問題はありません`);return}startSession(shuffle([...categoryTerms]),`${category}の全問題`,true,'','categoryAll')}
...
function categoryStatsHtml(){const rows=categoryStats();return `<section class="category-stats"><h2 class="category-stats-title">カテゴリ別成績</h2>${rows.map(row=>{const currentRound=row.round.completed+1,history=[...row.round.history].reverse();return `<div class="category-row"><div class="category-row-main"><button type="button" class="category-row-study" onclick="Glossary.startCategoryWeak('${esc(row.category)}')"><span class="category-row-heading"><span class="category-row-name">${esc(row.category)}</span><span class="category-row-round">${currentRound}周め</span></span>${categoryBarHtml(row)}</button><button type="button" class="category-row-reset" onclick="Glossary.resetCategory('${esc(row.category)}')" aria-label="${esc(row.category)}の学習履歴をリセット">↻</button></div>${history.length?`<details class="category-history"><summary>過去の周回成績</summary>${history.map(item=>`<div class="category-history-row"><span>${item.round}周め</span>${categoryBarHtml(item)}</div>`).join('')}</details>`:''}</div>`}).join('')}</section>`}
...
function resetCategory(category){const terms=data.terms.filter(term=>term.category===category);if(!terms.length)return;if(!confirm(`「${category}」の安心・勘・無理、解答履歴、周回履歴を削除します。この操作は元に戻せません。`))return;const ids=new Set(terms.map(term=>term.id));for(const term of terms){const bookmarked=Boolean(termState(term)?.bookmarked);delete learning[stateKey(term)];delete learning[`v135-${term.id}`];for(const number of term.legacyNumbers||[])delete learning[number];if(bookmarked)learning[stateKey(term)]={bookmarked:true}}const categoryMeta=learning[CATEGORY_ROUND_KEY];if(categoryMeta&&typeof categoryMeta==='object')delete categoryMeta[category];const meta=learning[TODAY_META_KEY];if(meta&&typeof meta==='object'){for(const id of ids){delete meta.dailyCounts?.[id];delete meta.lastShown?.[id]}meta.dailyTermIds=(meta.dailyTermIds||[]).filter(id=>!ids.has(Number(id)));meta.lastSession={ids:(meta.lastSession?.ids||[]).filter(id=>!ids.has(Number(id))),results:Object.fromEntries(Object.entries(meta.lastSession?.results||{}).filter(([id])=>!ids.has(Number(id))))};if(meta.activeSession?.ids?.some(id=>ids.has(Number(id))))meta.activeSession=null}normalizedKeys.clear();saveLearning();flushLearning();renderHome()}
