...
const publicHealthQuestions=new Map(data.terms.filter(term=>publicHealthCategories.has(term.category)).map(term=>[Number(term.id),JSON.parse(JSON.stringify(q[term.id]))]));
...
// Version 3.0.83: 最終内容監査で、論点・単位が不揃いだった消毒法の誤答を修正。
...
data.version='3.0.83';
quizData.version='3.0.83';
}
const APP_VERSION='3.0.83',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_META_KEY='__today10',REVIEW_DATE='2026-07-17';
...
function categoryStats(){const categoryOrder=['感染症法','感染症予防','消毒法','公衆衛生','地域保健法','健康増進法','消費者基本法','皮膚','毛髪','香粧品成分','カッティング'],byCategory=new Map();data.terms.forEach(term=>{if(!byCategory.has(term.category))byCategory.set(term.category,[]);byCategory.get(term.category).push(term)});const rows=[...byCategory.entries()].map(([category,terms])=>{const studiedTerms=terms.filter(t=>termState(t));const safe=studiedTerms.filter(t=>termState(t).status==='safe').length;const weak=studiedTerms.filter(t=>['caution','danger'].includes(statusBucket(t))).length;const studied=studiedTerms.length;const rate=studied?Math.round(safe/studied*100):null;return{category,total:terms.length,studied,safe,weak,rate}});rows.sort((a,b)=>categoryOrder.indexOf(a.category)-categoryOrder.indexOf(b.category));return rows}
function startCategoryWeak(category){const categoryTerms=data.terms.filter(term=>term.category===category),weakTerms=categoryTerms.filter(term=>['caution','danger'].includes(statusBucket(term))),hasLearning=categoryTerms.some(term=>statusBucket(term)!=='unlearned'),terms=weakTerms.length?weakTerms:hasLearning?[]:categoryTerms;if(!terms.length){showStatusMessage(`「${category}」に該当する復習対象はありません`);return}startSession(shuffle([...terms]),`${category}の復習`,true,'','categoryWeak')}
function categoryStatsHtml(){const rows=categoryStats();return `<section class="category-stats"><h2 class="category-stats-title">カテゴリ別成績</h2>${rows.map(row=>{const tone=row.rate===null?'none':row.rate<50?'danger':row.rate<75?'caution':'safe',rateLabel=row.rate===null?'未学習':`${row.rate}%`,countLabel=`（${row.total}）`,complete=row.rate===100?' is-complete':'';return `<div class="category-row tone-${tone}${complete}"><button type="button" class="category-row-study" onclick="Glossary.startCategoryWeak('${esc(row.category)}')"><span class="category-row-name">${esc(row.category)}</span><span class="category-row-bar"><span class="category-row-bar-fill" style="width:${row.rate===null?0:row.rate}%"></span></span><span class="category-row-meta">${rateLabel}${countLabel}</span></button><button type="button" class="category-row-reset" onclick="Glossary.resetCategory('${esc(row.category)}')" aria-label="${esc(row.category)}の学習履歴をリセット">↻</button></div>`}).join('')}</section>`}
...
function statusSummary(terms){const s=stats(terms),pct=s.total?Math.round(s.learned/s.total*100):0,action=status=>`Glossary.startStatus('${status}')`;return `<section class="summary-card"><div class="ring" style="--progress:${pct}%"><span>${pct}%</span></div><div><div class="summary-label">学習進捗</div><div class="summary-main">${s.learned} / ${s.total}語</div><div class="summary-sub">学習済み ${s.learned}語　未学習 ${s.unlearned}語<br>確認問題正答率 ${s.correct+s.wrong?s.rate+'％':'未解答'}</div></div></section><section class="status-grid"><button type="button" class="status-box safe" onclick="${action('safe')}" aria-label="安心 ${s.safe}語を復習"><span class="status-dot"></span>安心<em>${s.safe}語</em></button><button type="button" class="status-box caution" onclick="${action('caution')}" aria-label="注意 ${s.caution}語を復習"><span class="status-dot"></span>注意<em>${s.caution}語</em></button><button type="button" class="status-box danger" onclick="${action('danger')}" aria-label="無理 ${s.danger}語を復習"><span class="status-dot"></span>無理<em>${s.danger}語</em></button></section><div class="progress-line"><i style="width:${pct}%"></i></div>`}
...
function quizResultBoxes(){const s=stats(),items=[['safe','result-first','✓','安心',s.safe],['caution','result-second','✓','注意',s.caution],['danger','result-wrong','×','無理',s.danger]];return `<section class="quiz-home-results"><div style="grid-template-columns:repeat(3,minmax(0,1fr))">${items.map(([key,cls,icon,label,count])=>`<button type="button" class="${cls}" onclick="Glossary.startStatus('${key}')"><span aria-hidden="true">${icon}</span><strong>${label}</strong><em>${count}語</em></button>`).join('')}</div></section>`}
...
function listCards(terms){return terms.length?terms.map(term=>{const s=termState(term);return `<details class="term-card" id="term-${term.id}"><summary><span class="arrow">▶</span><span>${esc(term.name)}</span><span class="term-category">${esc(term.category)}</span></summary><div class="term-back">${termFields(term)}${sourceBlock(term)}<div class="assessment-inline">${assessmentButtons(term,s?.status,'inline')}</div></div></details>`}).join(''):'<div class="empty">対象の用語はありません。</div>'}
...
function performanceHtml(term,response){const perf=performanceFor(term),s=termState(term),symbols={first_correct:'○',second_correct:'△',wrong:'×',unable:'🔖'},label={safe:'安心候補',caution:'注意',danger:'無理',none:'データなし'}[perf.level],final={safe:'安心相当',caution:'注意相当',danger:'無理相当',unlearned:'未学習'}[finalPriority(term)],notice=response.unable?'自力回答不能として記録されています':s?.status==='safe'&&['caution','danger'].includes(perf.level)?'実績では要注意':['caution','danger','unable'].includes(s?.status)&&perf.level==='safe'?'実績は改善傾向です':'';return `<details class="performance-details"><summary>正答実績</summary><div>自己評価：${states[statusBucket(term)]||'未設定'}<br>実力評価：${label}<br>直近5回：${perf.rows.map(x=>symbols[resultType(x)]||'').join(' ')||'記録なし'}<br>1回目正解率：${perf.rate===null?'未算出':perf.rate+'％'}<br>復習優先度：${final}${notice?`<br><span class="performance-notice">${notice}</span>`:''}</div></details>`}
...
function todayAssessmentArea(term,response){return `<div class="assessment">${Object.entries(states).map(([key,label])=>`<button class="${key}${response.currentStatus===key?' selected':''}" onclick="Glossary.overrideToday('${key}')">${stateSymbol(key)} ${label}</button>`).join('')}</div>`}
...
function resetLearning(){if(!confirm('用語集の安心・注意・無理、解答履歴をすべて削除します。この操作は元に戻せません。'))return;learning={};try{localStorage.removeItem(STORAGE_KEY)}catch(_){}normalizedKeys.clear();renderHome()}
function resetCategory(category){const terms=data.terms.filter(term=>term.category===category);if(!terms.length)return;if(!confirm(`「${category}」の安心・注意・無理、解答履歴を削除します。この操作は元に戻せません。`))return;const ids=new Set(terms.map(term=>term.id));for(const term of terms){const bookmarked=Boolean(termState(term)?.bookmarked);delete learning[stateKey(term)];delete learning[`v135-${term.id}`];for(const number of term.legacyNumbers||[])delete learning[number];if(bookmarked)learning[stateKey(term)]={bookmarked:true}}const meta=learning[TODAY_META_KEY];if(meta&&typeof meta==='object'){for(const id of ids){delete meta.dailyCounts?.[id];delete meta.lastShown?.[id]}meta.dailyTermIds=(meta.dailyTermIds||[]).filter(id=>!ids.has(Number(id)));meta.lastSession={ids:(meta.lastSession?.ids||[]).filter(id=>!ids.has(Number(id))),results:Object.fromEntries(Object.entries(meta.lastSession?.results||{}).filter(([id])=>!ids.has(Number(id))))};if(meta.activeSession?.ids?.some(id=>ids.has(Number(id))))meta.activeSession=null}normalizedKeys.clear();saveLearning();flushLearning();renderHome()}
