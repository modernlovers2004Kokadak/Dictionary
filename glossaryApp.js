...
data.version='3.0.92';
quizData.version='3.0.92';
}
const APP_VERSION='3.0.92',STORAGE_KEY='riyoshi_glossary_learning_v1',TODAY_META_KEY='__today10',ROUND_META_KEY='__roundProgress',CATEGORY_ROUND_KEY='__categoryRounds',REVIEW_DATE='2026-07-17';
...
function categoryStatsHtml(){const rows=categoryStats(),total=rows.reduce((sum,row)=>sum+row.total,0),completed=rows.length?Math.min(...rows.map(row=>row.round.completed)):0,overall=rows.reduce((sum,row)=>({safe:sum.safe+row.safe,caution:sum.caution+row.caution,danger:sum.danger+row.danger,unlearned:sum.unlearned+row.unlearned,total:sum.total+row.total}),{safe:0,caution:0,danger:0,unlearned:0,total:0});return `<section class="category-stats"><h2 class="category-stats-title">カテゴリ別成績：計${total}問</h2><div class="category-overall"><span class="category-row-heading"><span class="category-row-name">全体</span><span class="category-row-round">${completed+1}周め</span></span>${categoryBarHtml(overall)}</div>${rows.map(row=>{const currentRound=row.round.completed+1,history=[...row.round.history].reverse();return `<div class="category-row"><div class="category-row-main"><button type="button" class="category-row-study" onclick="Glossary.startCategoryWeak('${esc(row.category)}')"><span class="category-row-heading"><span class="category-row-name">${esc(row.category)}：${row.total}問</span><span class="category-row-round">${currentRound}周め</span></span>${categoryBarHtml(row)}</button></div>${history.length?`<details class="category-history"><summary>過去の周回成績</summary>${history.map(item=>`<div class="category-history-row"><span>${item.round}周め</span>${categoryBarHtml(item)}</div>`).join('')}</details>`:''}</div>`}).join('')}</section>`}
